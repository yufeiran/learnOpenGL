...
void initLightingShader(Shader& lightingShader)
{
...
int main()
{
	...
	Model manModel("D://yufeiran//model//vampire//dancing_vampire.dae");
	std::string path = "D://yufeiran//model//vampire//dancing_vampire.dae";
	Animation danceAnimation(path,&manModel);
	...
	Shader animatorShader("../shaders/g_skeletalAnimation.vs", "../shaders/3_2_Model_1.fs");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		initLightingShader(lightingShader);
		initLightingShader(animatorShader);
		...
		animatorShader.use();
		animatorShader.setMat4("model", model);
		
		auto transforms = animator.GetFinalBoneMatrices();
		for(int i = 0; i < transforms.size(); ++i)
		{
		     animatorShader.setMat4("finalBonesMatrices[" + std::to_string(i) + "]",transforms[i]);
		}


		manModel.Draw(animatorShader);

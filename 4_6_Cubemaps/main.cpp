...
int main()
{
    ...
    // -------------------------
    Shader shader("../shaders/4_1_1.vs", "../shaders/4_1_1.fs");
    ...
    // cube with normal VAO
    unsigned int cubeNoramlVAO, cubeNoramlVBO;
    glGenVertexArrays(1, &cubeNoramlVAO);
    glGenBuffers(1, &cubeNoramlVBO);
    glBindVertexArray(cubeNoramlVAO);
    glBindBuffer(GL_ARRAY_BUFFER, cubeNoramlVBO);
    glBufferData(GL_ARRAY_BUFFER, sizeof(cubeWithNormalVertices), &cubeWithNormalVertices, GL_STATIC_DRAW);
    glEnableVertexAttribArray(0);
